...
def main(args):
    ...
    _, tns, tps, fns, fps = test(test_loader, model, batch_size, dist_type, result_write_path)
    print(f'true negative: {tns}, true positive: {tps}, false negative: {fns}, false positive: {fps}')


def test(test_loader, model, bs, dist_type, output_file='output-predictions.txt'):
    ...
    with torch.no_grad():
        for i, (input, target, image_name) in enumerate(test_loader):
            ...
            # compute output
            logits = model(input_val, targets=None, flag='test', dist_type=dist_type)
